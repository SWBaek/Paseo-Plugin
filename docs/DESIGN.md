...
마지막 대조일: 2026-09-21. 새 API 참조와 현재 네 플러그인 소스는 **0.9.0-beta.2** 기준이다. 아래 공통 시각 규칙은 0.7·0.8·0.9에 적용하고, 새 import·Settings·Modal 계약은 [0.8 이관](MIGRATION_0.8.md) 이후, Agent directory 소유와 0.9 버전 선언은 [0.9 이관](MIGRATION_0.9.md) 이후 적용한다.

## Rule precedence

규칙이 충돌하면 아래 순서를 따른다.

1. 대상 버전 공식 문서: [current quickstart](https://paseo.sh/docs/plugins)·[reference](https://paseo.sh/docs/plugins/reference), 0.8 유지보수는 [v0.8 reference](https://paseo.sh/docs/plugins/v0.8/reference), 0.7 유지보수는 [v0.7 reference](https://paseo.sh/docs/plugins/v0.7/reference)
...
## Buttons in final 0.8.0

Composer pill과 header button은 `button: { title, icon, label, behavior }`를 사용한다. Host가 label의 글꼴·색상·배치·접근성·pending/error 처리를 소유한다. Custom icon은 `PluginButtonIconProps`의 `size`와 `color`를 기본값으로 사용하며 의미 있는 상태 tone은 공개 theme token으로 표시한다. 기존 `Component` 안의 Text를 icon에 숨겨 넣지 않는다. 동적 label·title·disabled는 `registration.update()`로 갱신하고 `registration.remove()`로 정리한다. 다른 client `add*`의 cleanup 함수와 혼동하지 않는다.

Header의 compact label 생략과 overflow, Composer의 icon/label 및 chevron 없는 배치는 Paseo 소유다. 이 저장소는 정식판 이관에서 새 header button을 추가하지 않는다.
