...
이 문서는 Paseo `0.7.0-beta.1`에서 이 monorepo의 개별 플러그인을 Git source로 배포하는 절차를 설명합니다. Git source는 다른 daemon이나 PC에 배포하고 추적 ref를 업데이트하는 운영 경로입니다. 같은 컴퓨터에서 소스를 편집하는 동안에는 directory source 설치와 `plugin reload`를 사용하세요.
